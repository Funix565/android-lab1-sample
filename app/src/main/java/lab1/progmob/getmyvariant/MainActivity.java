package lab1.progmob.getmyvariant;
...
import lab1.progmob.getmyvariant.fragments.MenuFragment;
import lab1.progmob.getmyvariant.fragments.OptionsFragment;
import lab1.progmob.getmyvariant.fragments.ResultsFragment;
...
public class MainActivity extends AppCompatActivity implements AppContract {
    ...
    @Override
    public void toOptionsScreen(Fragment target, Student student) {
    ...
    public <T> void publish(T results) {
        ...
        List<ListenerInfo<?>> listeners = this.listeners.get(tag);
        if (listeners != null) {
            Iterator<ListenerInfo<?>> it = listeners.iterator();
            while (it.hasNext() && !it.next().tryPublish(results));
        }
    }

    @Override
    public <T> void registerListener(Fragment fragment, Class<T> clazz, ResponseListener<T> listener) {
        if (fragment.getTag() == null) {
            Log.e(TAG, "Fragment '" + fragment + "' doesn't have a tag");
            return;
        }

        List<ListenerInfo<?>> listeners = this.listeners.get(fragment.getTag());
        if (listeners == null) {
    ...
    private static class ListenerInfo<T> {
        ...
        private ListenerInfo(Class<T> clazz, ResponseListener<T> listener) {
            this.clazz = clazz;
